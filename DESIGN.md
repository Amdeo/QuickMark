...
- Pin/unpin from a result with its pin button or `Alt+P`. `Ctrl+1–N` opens the pinned sites (one key per pin, out of range does nothing); result rows are opened with `Shift+Ctrl+1–9` and their badges are the constant row number 1..9, so the two zones never collide and no offset is needed — `Ctrl` is never the opening modifier on macOS, where browsers reserve Cmd+digit for tab switching. With a query typed the strip is hidden but `Ctrl+digit` still opens that pinned site; `Shift+Ctrl+digit` always opens the result row. While the strip is visible a pinned URL is left out of the result rows, so each pin appears once and the row numbers shift accordingly; once a query hides the strip the pin returns to the list, because searching must still find it. Unpin is a small remove button revealed on hover or keyboard focus; with nothing pinned the bar renders no placeholder.
